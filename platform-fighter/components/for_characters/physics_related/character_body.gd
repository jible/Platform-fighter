...
func standard_movement_process():
	var acceleration = grounded_acceleration if player_body.IsGrounded() else aeriel_acceleration
	# If the player attempts to accelerate in the direction they are already traveling 
	if sign(input_dir) == sign(player_body.Velocity.x):
		# If they are below the max velocity, increase the velocity and cap it.
		if abs(player_body.Velocity.x) < max_horizontal_velocity:
...
func standard_drag_process():
	...
	if input_dir == 0 or abs(player_body.Velocity.x) > max_horizontal_velocity:
		var drag = grounded_drag if player_body.IsGrounded() else aeriel_drag
...
func can_jump() ->bool:
	return player_body.IsGrounded() or used_ariel_jumps < max_ariel_jumps

func jump():
	if player_body.IsGrounded():
		player_body.Velocity.y = -jump_vel
