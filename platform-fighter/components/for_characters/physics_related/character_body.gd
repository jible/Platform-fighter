...
func can_jump() ->bool:
	return is_on_floor() or used_ariel_jumps < max_ariel_jumps
...
func add_capped_velocity_impulse(impulse_vector: Vector2):
	...
	for axis in ["x", "y"]:
		...
		var added_x_velocities = velocity[axis] + impulse_vector[axis]
		# If velociy is added in the same direction:
		if sign(impulse_vector[axis]) == sign(velocity[axis]):
			# If starting velocity is within max velocity range:
			if abs(velocity[axis]) < max_velocity_vector[axis]:
				# Add velocity and cap it
				velocity[axis] = clamp(added_x_velocities, -max_velocity_vector[axis], max_velocity_vector[axis])
			...
			if sign(added_x_velocities) == sign(velocity[axis]):
				velocity[axis] = added_x_velocities
			# If adding the velocity changes the direction, add it and clamp it.
			else:
				velocity[axis] = clamp(added_x_velocities, -max_horizontal_velocity, max_horizontal_velocity)

func add_uncapped_velocity(velocity_impulse):
	velocity += velocity_impulse

func set_capped_velocity_on_axis(new_velocity, axis):
	velocity[axis] = clamp(new_velocity,-max_velocity_vector[axis],max_velocity_vector[axis])
